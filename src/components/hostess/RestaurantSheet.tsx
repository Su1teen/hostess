...
import { money, type Restaurant } from "@/data/hostess";
...
// simple 5x4 grid for floor plan
const tables = Array.from({ length: 20 }, (_, i) => ({
  id: i,
  seats: (i % 3) + 2,
  taken: [1, 4, 7, 8, 11, 15, 18].includes(i),
}));
    ...
    >
      ...
      <div className="relative h-72 w-full">
        ...
        <div className="absolute inset-x-0 top-12 flex justify-center gap-1 px-4">
          {r.gallery.map((_, i) => (
            <span key={i} className={`h-0.5 flex-1 max-w-16 rounded-full ${i === 0 ? "bg-white" : "bg-white/30"}`} />
          ))}
        </div>
        <div className="absolute inset-x-5 bottom-6 text-white">
          ...
          <p className="text-sm opacity-90">{r.cuisine} · {r.district}</p>
        </div>
      </div>
      ...
      <div className="px-5">
        ...
        <div className="no-scrollbar -mt-2 flex gap-2 overflow-x-auto pb-4">
          {r.tags.map((t) => (
            <span key={t} className="shrink-0 rounded-full bg-neutral-100 px-3 py-1.5 text-xs font-medium text-neutral-700">{t}</span>
          ))}
          <span className="shrink-0 rounded-full bg-neutral-100 px-3 py-1.5 text-xs font-medium text-neutral-700">Средний чек ~{(r.avgCheck/1000).toFixed(0)}k ₸</span>
        </div>
        ...
        {r.specials.length > 0 && (
          <div className="mt-6">
            ...
            <div className="no-scrollbar -mx-5 flex gap-3 overflow-x-auto px-5">
              {r.specials.map((s) => (
                <div key={s.id} className="relative w-64 shrink-0 overflow-hidden rounded-3xl bg-gradient-to-br from-primary to-[#EA580C] p-4 text-white">
                  <span className="absolute right-3 top-3 rounded-full bg-black/40 px-2 py-0.5 text-[10px] font-bold backdrop-blur">{s.special}</span>
                  <div className="flex items-start gap-3">
                    ...
                    <img src={s.image} className="h-20 w-20 rounded-2xl object-cover" alt="" />
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="mt-6">
          ...
          {r.menu.map((sec) => (
            <div key={sec.section} className="mb-4">
              <p className="mb-2 text-[11px] font-semibold uppercase tracking-widest text-neutral-500">{sec.section}</p>
              <div className="space-y-3">
                {sec.items.map((d) => (
                  <div key={d.id} className="relative flex overflow-hidden rounded-3xl bg-white hairline">
                    ...
                    <div className="relative w-32 shrink-0">
                      ...
                      <span className="absolute right-2 top-2 rounded-full bg-black/70 px-2 py-0.5 text-[10px] font-semibold text-white backdrop-blur">{d.weight} г</span>
                      <span className="absolute bottom-2 right-2 rounded-full bg-white/90 px-2 py-0.5 text-[10px] font-semibold text-neutral-900 backdrop-blur">
                        <Flame className="mr-0.5 -mt-0.5 inline h-2.5 w-2.5 text-primary" />{d.kcal} ккал
                      </span>
                    </div>
                  </div>
                ))}
        ...
        <div className="mt-4 rounded-3xl bg-neutral-50 p-4">
          ...
          <div className="flex items-center justify-between rounded-2xl bg-white p-3 hairline">
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => setGuests(Math.max(1, guests - 1))} className="grid h-9 w-9 place-items-center rounded-full bg-neutral-100"><Minus className="h-4 w-4" /></button>
              <span className="w-6 text-center text-sm font-semibold">{guests}</span>
              <button onClick={() => setGuests(Math.min(20, guests + 1))} className="grid h-9 w-9 place-items-center rounded-full bg-primary text-white"><Plus className="h-4 w-4" /></button>
            </div>
          </div>
            ...
            {zones.map((z) => (
              <button
                ...
                className={`flex-1 rounded-2xl px-3 py-2.5 text-xs font-medium ${
                  zone === z.key ? "bg-neutral-900 text-white" : "bg-white text-neutral-800 hairline"
                }`}
              >
                {z.label}
              </button>
          ...
          <div className="mt-3 rounded-3xl bg-white p-4 hairline">
            <div className="mb-3 flex items-center justify-between text-[11px] text-neutral-500">
              ...
              <span className="flex items-center gap-3">
                <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-neutral-300" />Занято</span>
                <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full border border-primary" />Свободно</span>
              </span>
            </div>
              ...
              {tables.map((t) => {
                ...
                return (
                  <button
                    ...
                    className={`aspect-square rounded-xl text-[10px] font-semibold transition ${
                      t.taken
            ...
            <div className="mt-3 grid grid-cols-3 gap-2 text-[10px] text-neutral-500">
              <div className="rounded-lg bg-neutral-100 py-2 text-center">Бар</div>
              <div className="rounded-lg bg-neutral-100 py-2 text-center">Сцена</div>
              <div className="rounded-lg bg-neutral-100 py-2 text-center">WC</div>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Sticky CTA */}
      <div className="pointer-events-none absolute inset-x-0 bottom-24 z-10 px-5">
        <button className="pointer-events-auto flex w-full items-center justify-between rounded-full bg-neutral-900 py-4 pl-6 pr-3 text-white shadow-xl">
          <span className="text-left">
            <span className="block text-[11px] opacity-70">{days[dayIdx]} · {time} · {guests} гостей</span>
...
export function SplitBillSheet({ onClose }: { onClose: () => void }) {
  ...
  const people = [
    { id: "А", name: "Айгерим", color: "bg-primary" },
    { id: "Д", name: "Данияр", color: "bg-neutral-900" },
    { id: "М", name: "Мадина", color: "bg-pink-500" },
    { id: "Е", name: "Ерлан", color: "bg-emerald-500" },
  ];
    ...
    >
      ...
      <div className="px-5 pt-6">
        ...
        <div className="mt-5 flex gap-3">
          {people.map((p) => (
            <div key={p.id} className="flex-1 rounded-2xl bg-neutral-50 p-3 text-center">
              <div className={`mx-auto grid h-10 w-10 place-items-center rounded-full ${p.color} text-sm font-semibold text-white`}>{p.id}</div>
              <p className="mt-2 text-[11px] text-neutral-500">{p.name}</p>
              <p className="text-sm font-semibold">
                {money(Math.round(items.filter(i => i.assigned === p.id).reduce((s, i) => s + i.price, 0) + items.find(i => i.assigned === "все")!.price / people.length))}
              </p>
            </div>
          ))}
        </div>
        ...
        <div className="mt-6 space-y-2">
          <p className="text-[11px] font-semibold uppercase tracking-widest text-neutral-500">Позиции</p>
          {items.map((it) => (
            <div key={it.id} className="flex items-center gap-3 rounded-2xl bg-white p-3 hairline">
              ...
              <div className="flex items-center gap-1">
                {it.assigned === "все" ? (
                  <span className="rounded-full bg-neutral-900 px-2.5 py-1 text-[10px] font-semibold text-white">На всех</span>
                ) : (
                  people
                    .filter((p) => p.id === it.assigned)
                    .map((p) => (
                      <span key={p.id} className={`grid h-7 w-7 place-items-center rounded-full ${p.color} text-[10px] font-semibold text-white`}>{p.id}</span>
                    ))
                )}
              </div>
            </div>
      ...
      <div className="absolute inset-x-0 bottom-24 z-10 px-5">
